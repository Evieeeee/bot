...
const processParamOrParams$1 = (val) => {
...
const binaryMethod = (operator) => {
    return function (val) {
        ...
        return new ctor({
            left: this.toNode(),
            operator,
            right: processParamOrParams$1(val)
        });
    };
};
const ternaryMethod = (operator, separator) => {
    return function (middle, right) {
        ...
        return new ctor({
            left: this.toNode(),
            operator,
            middle: processParam$1(middle),
            separator,
            right: processParam$1(right)
        });
    };
};
...
function ValueExpressionBaseMixin(Base) {
    class ValueExpressionBaseClass extends Base {
        ...
        at(index) {
            ...
            return new ctor(this.toNode(), processParam$1(index));
        }
        slice(start, end) {
            const ctor = classMap.get('SLICE');
            return new ctor(this.toNode(), processParam$1(start), processParam$1(end));
        }
        cast(dataType) {
        ...
        case(whenList, thenList, elseBranch) {
            ...
            if (undefined !== elseBranch) {
                elseBranch = processParam$1(elseBranch);
            }
            return new ctor({
        ...
        ;
        binaryOperator(operator, right) {
            const ctor = classMap.get('BINARY');
            return new ctor({
                left: this.toNode(),
                operator,
                right: processParamOrParams$1(right)
            });
        }
        ;
        ternaryOperator(operator, middle, separator, right) {
            const ctor = classMap.get('TERNARY');
            return new ctor({
                left: this.toNode(),
                operator,
...
const prefixUnaryOperator = (operator) => {
    return (left) => {
...
const postfixUnaryOperator = (operator) => {
    return (left) => {
...
const ternaryOperator = (operator, separator) => {
