...
function dateToStringUTC(date) {
    ...
    let ret = padStart_1.default(String(year), 4, '0') +
        '-' +
        padStart_1.default(String(date.getUTCMonth() + 1), 2, '0') +
        '-' +
        padStart_1.default(String(date.getUTCDate()), 2, '0') +
        'T' +
        padStart_1.default(String(date.getUTCHours()), 2, '0') +
        ':' +
        padStart_1.default(String(date.getUTCMinutes()), 2, '0') +
        ':' +
        padStart_1.default(String(date.getUTCSeconds()), 2, '0') +
        '.' +
        padStart_1.default(String(date.getUTCMilliseconds()), 3, '0');
    ret += 'Z';
    if (isBCYear)
        ret += ' BC';
    return ret;
...
class Postgres extends dialect_1.Dialect {
        ...
        if (null === value) {
            ...
            if (Array.isArray(value)) {
                if (this.myClass === Postgres) {
                    ...
                    if (value.length && 'object' === typeof value[0] && !isFunction_1.default(value[0].toISOString) && !Array.isArray(value[0])) {
                    ...
                    value = map_1.default(value, this._getParameterValue.bind(this));
                    value = `(${value.join(', ')})`;
                }
            }
    ...
    visitCase(caseNode) {
        assert_1.default(caseNode.whenList.length === caseNode.thenList.length);
        let text = '(CASE';
    ...
    visitColumn(columnNode) {
        ...
        if (this.visitingCreate || this.visitingAddColumn) {
            assert_1.default(columnNode.dataType, `dataType missing for column ${columnNode.name} (CREATE TABLE and ADD COLUMN statements require a dataType)`);
            ...
            if (!!columnNode.references) {
                assert_1.default.strictEqual(typeof columnNode.references, 'object', 'references is not a object for column ' +
                    columnNode.name +
                ...
                if (Object.keys(columnNode.references).length > 0) {
                    ...
                    assert_1.default(references.table, 'reference.table missing for column ' +
                        columnNode.name +
                        ' (REFERENCES statements within CREATE TABLE and ADD COLUMN statements' +
                        ' require a table and column)');
                    assert_1.default(references.column, 'reference.column missing for column ' +
                        columnNode.name +
                        ' (REFERENCES statements within CREATE TABLE and ADD COLUMN statements' +
                        ' require a table and column)');
                    txt.push(' REFERENCES ');
    ...
    visitForeignKey(foreignKeyNode) {
        const txt = [];
        if (this.visitingCreate) {
            assert_1.default(foreignKeyNode.table, 'Foreign table missing for table reference');
            assert_1.default(foreignKeyNode.columns, 'Columns missing for table reference');
            if (foreignKeyNode.refColumns !== undefined) {
                assert_1.default.strictEqual(foreignKeyNode.columns.length, foreignKeyNode.refColumns.length, 'Number of local columns and foreign columns differ in table reference');
            }
